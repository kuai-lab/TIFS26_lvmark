...
sys.path.append("..")
sys.path.append("../..")
from .diff_h264.diff_h264 import H264CompressorNet
# from diff_h264.utils import *
# from diff_h264.attack import ImageAugmentor

_ATTACK = ['c', 'r', 'g', 'b', 'n', 'e', 'j', 'h']
...
def attack_initializer(args, is_train, device):
    ...
    class ModelWrapper:
        ...
        def __call__(self, x):
            with torch.no_grad():
                ...
                x = rearrange(x.unsqueeze(0), 'b f c h w -> b c f h w')
                x = self.model(x)
                x = rearrange(x, 'b c f h w -> b f c h w').squeeze(0)
                return x
    
    h264_model = H264CompressorNet().to(device)
    h264_model.load_state_dict(torch.load('/data/minhyuk/cvpr2025/Dynamic_Wouaf_ours/attack_methods/model_weights_epoch_18.pth', map_location=device))
    h264_model.eval()
    ...
    attack_prob = 0.2 if is_train else 0.2
    if args.attack == 'all':
    ...
    modelWrapper = ModelWrapper(h264_model)
    attack = transforms.Compose([
        aug_list,
        transforms.Lambda(apply_with_prob(attack_prob if 'h' in args.attack else 0, modelWrapper)),
