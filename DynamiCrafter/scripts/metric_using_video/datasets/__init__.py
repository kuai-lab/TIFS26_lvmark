...
from .webvid_datasets import WebVid
...
def get_dataset(args):
    ...
    if args.dataset == 'ffs':
        ...
        transform_webvid = transforms.Compose([
                    video_transforms.ToTensorVideo(), # TCHW
                    # video_transforms.RandomHorizontalFlipVideo(),
                    video_transforms.CenterCropResizeVideo(args.image_size[0]),
                    transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True)
                ])
        return WebVid(args,dataset_path = args.train_data_path,transform=transform_webvid, temporal_sample=temporal_sample), WebVid(args,dataset_path = args.val_data_path, transform=transform_webvid, temporal_sample=temporal_sample)
    else:
