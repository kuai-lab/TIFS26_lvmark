...
class LatentDiffusion(DDPM):
    ...
    def decode_core(self, z, encoded_phis, affine_list, **kwargs):
        ...
        if not self.perframe_ae:    
            ...
            results = self.first_stage_model.decode(z,encoded_phis, affine_list, **kwargs)
        else:
            results = []
            for index in range(z.shape[0]):
                frame_z = 1. / self.scale_factor * z[index:index+1,:,:,:]
                # print(frame_z.shape)
                frame_result = self.first_stage_model.decode(frame_z, encoded_phis, affine_list,**kwargs)
                results.append(frame_result)
            results = torch.cat(results, dim=0)
    ...
    @torch.no_grad()
    def decode_first_stage(self, z, encoded_phis, affine_list ,**kwargs):
        return self.decode_core(z, encoded_phis, affine_list,**kwargs)
