...
def load_images_to_array(folder_path): # 0~255
    ...
    return images_np
def tLP(original_video, watermarked_video):
    model = dm.DistModel()
    model.initialize(model='net-lin',net='alex',use_gpu=True)
    gt = np.asarray(original_video.permute(0,2,3,1).float())
    watermarked = np.asarray(watermarked_video.permute(0,2,3,1).float())
    i = 0
    tLPs = []
