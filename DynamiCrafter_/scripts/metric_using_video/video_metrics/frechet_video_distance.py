...
def compute_our_fvd(videos_fake: np.ndarray, videos_real: np.ndarray, device: str='cuda') -> float:
    ...
    detector = torch.load('/data/minhyuk/cvpr2025/Dynamic_Wouaf_ours/DynamiCrafter/scripts/metric_using_video/video_metrics/i3d_torchscript.pt').eval().to(device)
    videos_fake = torch.from_numpy(videos_fake).permute(0, 4, 1, 2, 3).to(device)
    videos_real = torch.from_numpy(videos_real).permute(0, 4, 1, 2, 3).to(device) # 1 3 T H W
...
def fvd(original_video, watermarked_video):
    transform = Resize((224, 224))
    # print(transform(original_video).shape)
    gt = np.asarray(transform(original_video).permute(0,2,3,1).unsqueeze(0).float())# 1 T C H W
    watermarked = np.asarray(transform(watermarked_video).permute(0,2,3,1).unsqueeze(0).float()) # 1 T C H W
