...
def get_bit_accs(device, watermarked_video, original_video, msg_decoder: nn.Module, key: torch.Tensor, batch_size: int = 16, attacks: dict = {}):
    ...
    for name, attack in attacks.items():
        imgs_aug = attack(watermarked_video.to(device)) 
        imgs_aug = imgs_aug.to(device)  # [16, 3, 256, 256]
        all_decoded = []

        try:
            for frame_idx in range(imgs_aug.shape[0]):
                frame = imgs_aug[frame_idx].unsqueeze(0)
                # decoded = msg_decoder.detect(frame)["preds"]
                # decoded = decoded.to(device)

                # mask_preds = F.sigmoid(decoded[:, 0, :, :])
                # bit_preds = decoded[:, 1:, :, :]

                # decoded = msg_predict_inference(bit_preds, mask_preds).cpu().float()
                decoded = msg_decoder(frame) 
                decoded = (decoded > 0.5).int()
        ...
        if 'frame_drop' in name or name in ['comb_h264_framedrop_crop', 'comb_h264_framedrop_rot']:
            _, _, bit_accs = bch_error_correction_batch(key[:decoded.shape[0]], decoded,bch_bits=11)
        else:
            _, _, bit_accs = bch_error_correction_batch(key, decoded,bch_bits=11)
